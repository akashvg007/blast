...
import { Text, View, StyleSheet, ScrollView, Image, BackHandler } from 'react-native';
...
import { addToMessage, getChatsFromLocal, getUpdatedMessage } from '../helper/logicHelper';
import EmojiSelector, { Categories } from "react-native-emoji-selector";

export default function ChatScreen({ photo, name, contact, handleBack, ...props }) {
    ...
    const handleSend = async () => {
        ...
        await addToMessage({ recipient: name, text: textInput, sender: myphone, status: 1 })
        ...
        <View style={styles.container}>
            ...
            <ScrollView style={{ backgroundColor: '#f1f1f1' }}
                ...
                <View style={{ flexDirection: 'column-reverse' }}>
                    {
                        localData && localData?.map((chat, idx) => {
                            const date = moment(chat.time).format("MMM Do YY");
                            const curr = moment(chat.time).format("MMM Do YY");
                            let prev = "";
                            if (localData[idx + 1]) prev = moment(localData[idx + 1].time).format("MMM Do YY");
                            const showDate = () => {
                                const compareString = curr.localeCompare(prev)
                                if (compareString !== 0) {
                                    return (<View style={styles.chatDate}>
                            ...
                            return (
                                <View key={idx} style={styles.chatContainer}>
                                    {showDate()}
                                    <View style={[styles.chatRegion, chat.from === myphone ? styles.rightSide : styles.leftSide]}>
                                        <View style={[styles.chat, chat.from === myphone ? styles.rightChat : styles.leftChat]}>
                                            <Text style={styles.message}>{chat.msg}</Text>
                                            <View style={[styles.time, chat.from === myphone ? styles.rightSide : styles.leftSide]}>
                                                <Text>
            ...
            <View>
                <TextInput
                    ...
                    value={textInput}
                    left={<TextInput.Icon style={styles.emoji} onPress={handleEmoji} name={require('../assets/emoticon-happy.png')} />}
